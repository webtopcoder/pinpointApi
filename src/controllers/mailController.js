...
const mailController = {
  compose: async (req, res) => {
    try {
      ...
      const file_paths = [];
      if (Validator.isEmpty(to) || Validator.isEmpty(message)) {
      ...
      if (req.files !== null) {
        ...
        if (files.length === undefined) {
          ...
          const filename =
            files.name.substring(0, files.name.length - 4) +
            "-" +
            new Date().getTime() +
            ext;
          ...
          for (var i = 0; i < files.length; i++) {
            ...
            const filename =
              files[i].name.substring(0, files[i].name.length - 4) +
              "-" +
              new Date().getTime() +
              "-" +
              i +
  ...
  inbox: async (req, res) => {
    try {
      ...
      const total = await Mail.aggregate([
        {
          $match: {
            to: to_user,
            type: "usual",
  ...
  update: async (req, res) => {
    try {
      ...
      if (Validator.isEmpty(_id) || Validator.isEmpty(actionType)) {
      ...
      if (actionType === "delete") {
        if (is_read) {
          // inbox
          update = await Mail.updateMany(
            { from: { $in: _id }, to: req.user.email },
            { to_is_deleted: true }
          );
        } // sent
        else {
          update = await Mail.updateMany(
            { to: { $in: _id }, from: req.user.email },
        ...
        if (is_read) message = "You have just read the email";
