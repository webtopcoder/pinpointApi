...
const profileController = {
  ...
  get: async (req, res) => {
    try {
      ...
      if (!Validator.isEmpty(user)) {
        ...
        return res.json({ success: false, msg: "There is no profile data" });
    } catch (error) {
      console.log(error);
      return res.json({ success: false, msg: msg.server.error });
    }
  ...
  avatar: async (req, res) => {
    try {
      if (req.files.avatar !== null) {
        ...
        const files = avatar;
        const ext = files.name.substring(
          files.name.length - 4,
          files.name.length
        );
        ...
        let update = await User.updateOne(
          { email },
          { profile: { ...user.profile, avatar: filename } }
        );
        if (update.modifiedCount > 0) {
          return res.json({
            success: true,
            msg: "Updated your avatar",
            avatar: filename,
          });
        } else return res.json({ success: false, msg: "Something went wrong" });
      }
  ...
  post: async (req, res) => {
    try {
      ...
      if (Validator.isEmpty(content) || Validator.isEmpty(userid)) {
        return res.json({ success: false, msg: "Bad Request" });
      }
      ...
      if (req.files !== null) {
        ...
        if (files.length === undefined) {
          ...
          for (var i = 0; i < files.length; i++) {
            const ext = files[i].name.substring(
  ...
  getHeaderInfo: async (req, res) => {
    try {
      let { userid } = req.params;
      let userProfile = await User.findOne({ _id: userid });
      let follow = await Post.find({
        to: userProfile.email,
        type: "follow",
      }).countDocuments();
      let is_follow = false;
      if (follow > 0) is_follow = true;
      if (!Validator.isEmpty(userProfile)) {
        return res.json({
          success: true,
          profile: {
  ...
  followers: async (req, res) => {
    try {
      ...
      if (Validator.isEmpty(page)) page = 0;
  ...
  shootOut: async (req, res) => {
    try {
      ...
      const followers = await Post.aggregate([
        ...
        {
          $match: {
            "to_user._id": mongoose.Types.ObjectId(userid),
            type: "follow",
          },
        },
        {
  ...
  like: async (req, res) => {
    try {
      ...
      if (like.indexOf(req.user.id) > -1) {
        return res.json({
          success: false,
          msg: `You've already like this post`,
        });
      }
